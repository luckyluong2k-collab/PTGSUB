...
function renderActivity(data){const activity=document.getElementById("adviceActivity");if(!activity)return;const units=Array.isArray(data.units)?data.units:[];const interestedCode=String(data.interestedUnitCode||"").trim().toUpperCase();const interested=interestedCode&&units.some((unit)=>String(unit?.code||"").trim().toUpperCase()===interestedCode);const lastViewedMs=timestampMs(data.lastViewedAt);const recentlyViewed=lastViewedMs>0&&Date.now()-lastViewedMs>=0&&Date.now()-lastViewedMs<=24*60*60*1000;const views=Math.max(0,Number(data.viewCount)||0);if(!interested&&!recentlyViewed){activity.hidden=true;return}let title="Báo giá vừa có lượt mở gần đây";let message=views>0?`Hồ sơ này đã được mở ${views} lượt. Tình trạng căn và chính sách có thể thay đổi theo thời điểm.`:"Tình trạng căn và chính sách có thể thay đổi theo thời điểm.";if(interested){title=`Căn ${interestedCode} đang được khách quan tâm`;message=recentlyViewed?`Báo giá vừa có lượt mở gần đây và căn ${interestedCode} đã được sale ghi nhận quan tâm.`:`Căn ${interestedCode} đã được sale ghi nhận quan tâm. Hãy liên hệ để kiểm tra tình trạng mới nhất.`}setText("adviceActivityTitle",title);setText("adviceActivityMessage",message);activity.hidden=false}
